/**
 * HubSpot OAuth callback — matches the registered redirect URL:
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  if (!code || !siteId) {
    return NextResponse.redirect(
      `${FRONTEND}/?oauth=error&reason=missing_params`,
    );
  }
  ...
  try {
    ...
    console.log(
      `[OAuth] Connected HubSpot portal ${tokenInfo.hub_id} for site ${siteId}`,
    );
    ...
    return NextResponse.redirect(
      `${FRONTEND}/?oauth=error&reason=server_error`,
    );
  }
}
